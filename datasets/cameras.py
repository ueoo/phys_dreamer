...
class Camera(nn.Module):
    ...
    def interpolate(self, next_camera, steps=24):
        interpolated_R_t = interpolate_cameras(self.R, self.T, next_camera.R, next_camera.T, steps)
        ...
        for i, (R, t) in enumerate(interpolated_R_t):
            time_stamp = self.time_stamp + (next_camera.time_stamp - self.time_stamp) * (i) / steps
            cam = Camera(
                R,
                t,
                self.FoVx,
