...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, img_hw=None, scale_x_angle=1.0):
    cam_infos = []
...
class MultiviewVideoDataset(Dataset):
    ...
    def __init__(
        ...
        for cam in self.camera_list:
            ...
            if os.path.exists(mask_path):
                ...
                mask = np.ones((self.resolution[0], self.resolution[1], 1), dtype=np.float32)
    ...
    def _parse_dataset(self, data_dir):
        ...
        if os.path.exists(camera_transform_file):
            ...
            camera_list, meta_dict = self._read_camera_transforms(camera_transform_file, img_hw=self.resolution)
            # assume test_meta_dict is the same as meta_dict
            test_camera_list, _ = self._read_camera_transforms(camera_transform_file_test, img_hw=self.resolution)
            print("read {} cameras".format(len(camera_list)))
        else:
            assert os.path.exists(os.path.join(data_dir, "sparse/0")), "colmap sparse folder not found!"
        ...
        self._num_frames = meta_dict["num_frames"]  # number of timestamps in the dataset
    ...
    def _read_camera_transforms_colmap(
        ...
        if eval:
            ret_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
        else:
