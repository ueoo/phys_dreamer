...
from diff_warp_utils import MPMModelStruct, MPMStateStruct
from fire import Fire
from mpm_solver_warp_diff import MPM_Simulator_WARPDiff
from mpm_utils import *
from run_gaussian_static import get_volume, load_gaussians
...
def test(input_dir, output_dir=None, fps=6, device="cuda"):
    ...
    nu_tensor = (torch.ones(n_particles) * material_params["nu"]).contiguous().to(device)
    mpm_model.from_torch(E_tensor, nu_tensor, device=device, requires_grad=True)

    mpm_solver = MPM_Simulator_WARPDiff(n_particles, n_grid=100, grid_lim=1.0, device=device)
