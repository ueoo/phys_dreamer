...
def run_mpm_gaussian(input_dir, output_dir=None, fps=6, device="cuda"):
    ...
    static_center_point = torch.from_numpy(gaussian_dict["satic_center_point"]).float().to(device)
    max_static_offset = torch.from_numpy(gaussian_dict["max_static_offset"]).float().to(device)
    ...
    save_dict = {
        "pos_init": mpm_solver.export_particle_x_to_torch().clone().detach().cpu().numpy(),
        "velo_init": mpm_solver.export_particle_v_to_torch().clone().detach().cpu().numpy(),
        "pos_list": [],
    }
    ...
    save_name += "_timestep_{}_vs{}_totaltime_{}".format(time_step, velocity_scaling, total_time)
...
def render_gaussians(
    ...
    from gaussian_3d.gaussian_renderer.render import render_gaussian
    from gaussian_3d.scene import GaussianModel
    ...
    from utils.io_utils import save_video_imageio
