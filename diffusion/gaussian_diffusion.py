...
class GaussianDiffusion:
    ...
    def ddim_sample(
        ...
        if y0 is not None and mask is not None:
            ...
            if is_mask_t0:
                ...
                nonzero_mask = (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))  # no noise when t == 0
                out["pred_xstart"] = (mask * y0 + (1 - mask) * out["pred_xstart"]) * nonzero_mask + out[
                    "pred_xstart"
                ] * (1 - nonzero_mask)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            target_tx, target_ty, target_tz = th.split(target, target.shape[-1] // 3, dim=-1)
            output_tx, output_ty, output_tz = th.split(model_output, model_output.shape[-1] // 3, dim=-1)
            ...
            if "vb" in terms:
                terms["loss"] = terms["mse_tx"] + terms["mse_ty"] + terms["mse_tz"] + terms["vb"]
