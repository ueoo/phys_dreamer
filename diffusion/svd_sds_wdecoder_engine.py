...
from utils.svd_helpper import get_batch, get_unique_embedder_keys_from_conditioner
...
class SVDWDecSDSEngine(pl.LightningModule):
    ...
    def forward_with_encoder_chunk(self, batch, chunk_size=2, sample_time_range=[0.02, 0.98]):
        with torch.no_grad():
            x = self.get_input(batch)  # [BT, 3, H, W]
            T = batch["num_video_frames"]
            batch_size = x.shape[0] // T
        ...
        for n in range(num_chunks):
            ...
            target_chunk = (z_chunk_recompute - sds_grad[n * chunk_size : end_ind]).detach()
    ...
    def edm_sds(self, input_x, extra_input, sample_time_range=[0.02, 0.98]):
        ...
        additional_model_inputs["image_only_indicator"] = torch.zeros(int(2 * batch_size), num_frames).to(device)
        ...
        noised_input = self.loss_fn.get_noised_input(sigmas_bc, noise, input_x)  # [BT, C, H, W]

        # step-3: prepare conditional and unconditional inputs
        # [2BT, C, H, W], [2BT]
        bathced_xt, bathced_sigmas, bathched_c = self.sampler.guider.prepare_inputs(noised_input, sigmas, c, uc)
    ...
    def edm_sds_multistep(self, input_x, extra_input, sample_time_range=[0.02, 0.84], num_step=4, total_steps=25):
        ...
        for i in range(num_step):
            ...
            denoised = self.sampler_step(
                sigmas, noised_input, c, uc, num_frames=num_frames, additional_model_inputs=additional_model_inputs
            )
            prev_sigmas = sigmas
            sigma_sum += sigmas_bc
    ...
    def sampler_step(self, sigma, noised_input, c, uc=None, num_frames=None, additional_model_inputs=None):
        ...
        # step-3: prepare conditional and unconditional inputs
        # [2BT, C, H, W], [2BT]
        bathced_xt, bathced_sigmas, bathched_c = self.sampler.guider.prepare_inputs(noised_input, sigma, c, uc)
        # bathched_c["crossattn"] => [2BT, 1, C] ;   bathched_c["concat"] => [2BT, C, H, W]; bathched_c["vector"] => [2BT, C_feat]
