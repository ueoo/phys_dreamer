...
def convert_gaussian_to_mesh(gaussian_path, thresh=0.1, save_path=None):
    ...
    mesh = gaussians.extract_mesh(save_path, density_thresh=thresh, resolution=128, decimate_target=1e5)

    mesh.write(save_path)


def internal_filling(gaussian_path, thresh=2.0, save_path=None, resolution=256, num_pts=4):
    if save_path is None:
        dir_path = os.path.dirname(gaussian_path)
    ...
    occ = gaussians.extract_fields(resolution=resolution, num_blocks=16, relax_ratio=1.5).detach().cpu().numpy()
