...
from warp_mpm.mpm_utils import (
    aggregate_grad,
    compute_Closs_with_grad,
    compute_Floss_with_grad,
    compute_position_l2_loss,
    compute_posloss_with_grad,
    compute_veloloss_with_grad,
)
from warp_mpm.warp_utils import CondTape, MyTape, from_torch_safe
    ...
    def forward(
        ...
        if static_pos is None:
            ...
            init_xyzs_wp = from_torch_safe(
                init_pos.clone().detach().contiguous(), dtype=wp.vec3, requires_grad=requires_grad
            )
...
class MPMDifferentiableSimulationWCheckpoint(autograd.Function):
    ...
    def backward(
        ctx,
        out_pos_grad: Float[Tensor, "n 3"],
        out_velo_grad: Float[Tensor, "n 3"],
        out_F_grad: Float[Tensor, "n 9"],
        out_C_grad: Float[Tensor, "n 9"],
    ):
        ...
        starting_state.reset_density(
            tensor_density=particle_density, selection_mask=query_mask, device=device, requires_grad=True
        )
...
class MPMDifferentiableSimulationClean(autograd.Function):
    ...
    def forward(
        ...
        Float[Tensor, "n 3"], Float[Tensor, "n 3"], Float[Tensor, "n 9"], Float[Tensor, "n 9"], Float[Tensor, "n 6"]
    ]:
        """
        Args:
            query_mask: [n] 0 or 1.  1 means the density or young's modulus, or poisson'ratio of this particle can change.
