...
class Trainer:
    ...
    def setup_simulation(self, dataset_dir, grid_size=100):
        ...
        if os.path.exists(filled_in_points_path):
            ...
            fill_xyzs = fill_xyzs[np.random.choice(fill_xyzs.shape[0], int(fill_xyzs.shape[0] * 1.0), replace=False)]
    ...
    def demo(
        ...
        if apply_force:
            ...
            center_point = torch.from_numpy(self.demo_cfg["center_point"]).to(device).float()
            ...
            self.add_constant_force(center_point, force_radius, force, delta_time, 0.0, force_duration)
        ...
        if not static_camera:
            interpolated_cameras = get_camera_trajectory(cam, num_pos, self.demo_cfg["camera_cfg"], self.test_dataset)
        else:
            interpolated_cameras = [cam] * num_pos

        if not do_render_force:
            video_array, moving_part_video = render_gaussian_seq_w_mask_with_disp_for_figure(
        ...
        if not static_camera:
            save_name = save_name + "_movingcamera" + "_camid_{}".format(self.demo_cfg["cam_id"])
    ...
    def compute_metric(self, exp_name, result_dir):
        ...
        save_path = os.path.join(
            result_dir,
            exp_name + "_jelly_densi2k_video_substep_{}_grid_{}".format(substep, self.args.grid_size) + ".mp4",
        )
