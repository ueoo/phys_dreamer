...
class Trainer:
    def __init__(self, args):
        ...
        args.wandb_name += "decay_{}_substep_{}_{}_lr_{}_tv_{}_iters_{}_sw_{}_cw_{}".format(
            args.loss_decay,
            args.substep,
            args.model,
            args.lr,
        ...
        if args.checkpoint_path is not None:
            ...
            self.velo_optimizer, self.velo_scheduler = accelerator.prepare(self.velo_optimizer, self.velo_scheduler)
    ...
    def init_trainable_params(self):
        ...
        young_numpy = np.exp(np.random.uniform(np.log(1e-3), np.log(1e3))).astype(np.float32)
        young_numpy = 1e6 * 1.0

        young_modulus = torch.tensor(young_numpy, dtype=torch.float32).to(self.accelerator.device)

        poisson_numpy = np.random.uniform(0.1, 0.4)
        poisson_ratio = torch.tensor(poisson_numpy, dtype=torch.float32).to(self.accelerator.device)
    ...
    def setup_simulation(self, dataset_dir, grid_size=100):
        ...
        sim_cov = self.render_params.gaussians.get_covariance()[self.sim_mask_in_raw_gaussian, :].detach().clone()
        ...
        if os.path.exists(filled_in_points_path):
            ...
            fill_xyzs = fill_xyzs[np.random.choice(fill_xyzs.shape[0], int(fill_xyzs.shape[0] * 0.25), replace=False)]
        ...
        sim_aabb = torch.stack([torch.min(sim_xyzs, dim=0)[0], torch.max(sim_xyzs, dim=0)[0]], dim=0)
        sim_aabb = (sim_aabb - torch.mean(sim_aabb, dim=0, keepdim=True)) * 1.2 + torch.mean(
            sim_aabb, dim=0, keepdim=True
        )
        ...
        mpm_solver = MPMWARPDiff(num_particles, n_grid=grid_size, grid_lim=1.0, device=device)
        ...
        if os.path.exists(moving_pts_path):
            ...
            grid_freeze_mask = apply_grid_bc_w_freeze_pts(grid_size, 1.0, freeze_pts, mpm_solver)
            ...
            print(f"moving_pts_path {moving_pts_path} not found!")
        ...
        density = torch.ones_like(self.particle_init_position[..., 0]) * material_params["density"]
        youngs_modulus = torch.ones_like(self.particle_init_position[..., 0]) * self.E_nu_list[0].detach()
        ...
        if os.path.exists(stem_pts_path):
            ...
            no_stem_mask = find_far_points(sim_xyzs, stem_pts, thres=2.0 / grid_size).bool()
        ...
        mpm_solver.set_E_nu_from_torch(mpm_model, youngs_modulus.clone(), poisson_ratio.clone(), device)
        ...
        self.velo_fields = create_spatial_fields(self.args, 3, sim_aabb, add_entropy=False)
    ...
    def set_simulation_state(
        ...
        if use_precompute_F:
            ...
            self.mpm_solver.restart_and_compute_F_C(self.mpm_model, self.mpm_state, init_xyzs_wp, device=device)
        else:
            self.mpm_state.reset_state(
                init_xyzs.clone(),
                None,
    ...
    def train_one_step(self):
        ...
        if not do_velo_opt:
            stop_velo_opt_thres = 0  # stop velocity optimization if we are loading from checkpoint
        ...
        for time_idx in range(start_time_idx, window_size):
            ...
            density, youngs_padded, init_velocity, query_mask = self.get_density_velocity(time_stamp, device)
            ...
            extra_no_grad_step = max(0, (time_idx - self.args.grad_window + 1) * num_substeps)
            if do_velo_opt:
                extra_no_grad_step = 0

            num_step_with_grad = num_substeps * (time_idx + 1) - extra_no_grad_step

            # Use checkpointed simulation to reduce GPU memory (avoids storing all intermediate states)
            init_F = torch.eye(3, device=device).view(1, 3, 3).repeat(num_particles, 1, 1)
            init_C = torch.zeros((num_particles, 3, 3), device=device)
            particle_pos, _, _, _ = MPMDifferentiableSimulationWCheckpoint.apply(
                self.mpm_solver,
                self.mpm_state,
                self.mpm_model,
                substep_size,
            ...
            loss = loss * (self.args.loss_decay**time_idx) + sm_loss * self.tv_loss_weight
        ...
        if accelerator.is_main_process and (self.step % self.wandb_iters == 0):
            with torch.no_grad():
                ...
                sim_video_torch = torch.from_numpy(simulated_video).float().to(device) / 255.0
                ...
                if self.step % int(self.wandb_iters) == 0:
                    ...
                    simulated_video = self.inference(cam, num_sec=3, substep=self.args.substep)
                    wandb_dict["inference_video_t3"] = wandb.Video(
                        simulated_video,
                        fps=simulated_video.shape[0] // 3,
                    )

                    simulated_video = self.inference(cam, velo_scaling=5.0, num_sec=3, substep=self.args.substep)
    ...
    @torch.no_grad()
    def inference(self, cam, velo_scaling=1.0, num_sec=1, nu=None, young_scaling=1.0, substep=20):
    ...
    def eval(self):
        ...
        save_path = os.path.join(
            output_dir,
            "eval_fill2k_video_nu_{}_ys_{}_substep_{}_grid_{}".format(nu, young_scaling, substep, self.args.grid_size)
            + ".gif",
        )
