...
from gaussian_3d.gaussian_renderer.render import render_arrow_in_screen, render_gaussian
from gaussian_3d.utils.rigid_body_utils import (
    get_rigid_transform,
    matrix_to_quaternion,
    quaternion_multiply,
)
from utils.camera_utils import generate_spiral_path
from utils.svd_helpper import load_model_from_config
...
def setup_boundary_condition(xyzs_over_time: torch.Tensor, mpm_solver, mpm_state, num_filled=0):
    ...
    if num_filled > 0:
        freeze_mask = torch.cat([freeze_mask, freeze_mask.new_zeros(num_filled).type(torch.int)], dim=0)
    ...
    mpm_solver.enforce_particle_velocity_by_mask(mpm_state, freeze_mask, free_velocity, start_time=0, end_time=100000)
...
def setup_plannar_boundary_condition(
    ...
    # [n_particles]
    plane_dist = torch.abs(torch.sum((gaussian_xyz - plane_mean.unsqueeze(0)) * plane_normal.unsqueeze(0), dim=-1))
    # [n_particles]
...
def setup_boundary_condition_with_points(xyzs, selected_points, mpm_solver, mpm_state, thres=0.05):
    """
    Args:
        xyzs: [N, 3]
        selected_points: [M, 3]
    ...
    mpm_solver.enforce_particle_velocity_by_mask(mpm_state, freeze_mask, free_velocity, start_time=0, end_time=1000000)
...
def render_single_view_video(
    ...
    for time_stamp in time_stamps:
        if not render_flow:
            new_gaussians = render_params.gaussians.apply_se3_fields(motion_model, time_stamp)
            if query_mask is not None:
                new_gaussians._xyz = new_gaussians._xyz * query_mask.unsqueeze(-1) + render_params.gaussians._xyz * (
                    1 - query_mask.unsqueeze(-1)
                )
                new_gaussians._rotation = new_gaussians._rotation * query_mask.unsqueeze(
                    -1
            ...
            inp_time = torch.ones_like(render_params.gaussians._xyz[:, 0:1]) * time_stamp
...
def render_gaussian_seq_w_mask(cam, render_params, gaussian_pos_list, gaussian_cov_list, update_mask):
...
def render_gaussian_seq_w_mask_with_disp(cam, render_params, orign_points, top_k_index, disp_list, update_mask):
    """
    Args:
    ...
    for i in range(len(disp_list)):
        ...
        if isinstance(cam, list):
            ...
            render_cam = cam
        disp = disp_list[i]
        new_xyz, new_rotation = interpolate_points_w_R(query_pts, query_rotation, orign_points, disp, top_k_index)
...
def render_gaussian_seq_w_mask_cam_seq(cam_list, render_params, gaussian_pos_list, gaussian_cov_list, update_mask):
...
def apply_grid_bc_w_freeze_pts(grid_size, grid_lim, freeze_pts, mpm_solver):
    ...
    grid_pts_cnt = torch.zeros((grid_size, grid_size, grid_size), dtype=torch.int32, device=device)
...
def interpolate_points_w_R(query_points, query_rotation, drive_origin_pts, drive_displacement, top_k_index):
    """
    Args:
        query_points: [n, 3]
...
def create_camera_path(
    ...
    lookat_pt = focus_pt
    render_poses = generate_spiral_path(C2W, radius, lookat_pt, up, n_frames, n_rots, y_scale)
