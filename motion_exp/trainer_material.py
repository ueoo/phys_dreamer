...
class Trainer:
    ...
    def setup_simulation(self, dataset_dir, grid_size=100):
        ...
        if os.path.exists(moving_pts_path):
            ...
            freeze_mask = find_far_points(sim_xyzs, moving_pts, thres=0.5 / grid_size).bool()
    ...
    def get_simulation_input(self, device):
        ...
        particle_F = torch.repeat_interleave(I_mat[None, ...], initial_position_time0.shape[0], dim=0)
    ...
    def train_one_step(self):
        ...
        for start_time_idx in range(0, window_size, temporal_stride):
            ...
            if checkpoint_steps > 0 and checkpoint_steps < num_step_with_grad:
                for time_step in range(0, num_step_with_grad, checkpoint_steps):
                    ...
                    if num_step == 0:
                        break
                    particle_pos, particle_velo, particle_F, particle_C = MPMDifferentiableSimulationWCheckpoint.apply(
                        self.mpm_solver,
                        self.mpm_state,
                        self.mpm_model,
                        substep_size,
            ...
            sm_loss = sm_velo_loss + sm_spatial_loss  # typically 20 times larger than rendering loss
        ...
        if accelerator.is_main_process and (self.step % self.wandb_iters == 0):
            with torch.no_grad():
                ...
                young_color_full = torch.ones_like(self.render_params.gaussians._xyz[:, 0])

                young_color_full[self.sim_mask_in_raw_gaussian] = young_color
                young_color = torch.stack([young_color_full, young_color_full, young_color_full], dim=-1)
                ...
                young_img = (young_img.detach().cpu().numpy() * 255.0).astype(np.uint8).transpose(1, 2, 0)
                ...
                if self.step % int(10 * self.wandb_iters) == 0:
                    ...
                    simulated_video = self.inference(cam, velo_scaling=5.0, num_sec=3, substep=num_substeps)
    ...
    def inference(
        ...
        self.mpm_state.reset_density(density.clone(), query_mask, device, update_mass=True)
        self.mpm_solver.set_E_nu_from_torch(self.mpm_model, youngs_modulus.clone(), poisson.clone(), device)
    ...
    def demo(
        ...
        save_path = os.path.join(
            save_name
            + "_jelly_video_substep_{}_grid_{}_evalys_{}".format(substep, self.args.grid_size, eval_ys)
            + ".gif"
        )
