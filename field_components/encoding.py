...
from losses.smoothness_loss import compute_plane_smoothness, compute_plane_tv
...
class TemporalNeRFEncoding(nn.Module):
    ...
    def forward(
        ...
        freqs = torch.linspace(self.min_freq, self.max_freq, self.num_frequencies, device=in_tensor.device)
        if self.log_scale:
            freqs = 2**freqs
