...
class MPMWARPDiff(object):
    ...
    def set_parameters_dict(self, mpm_model, mpm_state, kwargs={}, device="cuda:0"):
        ...
        if "g" in kwargs:
            mpm_model.gravitational_accelaration = wp.vec3(kwargs["g"][0], kwargs["g"][1], kwargs["g"][2])
    ...
    def p2g2p_differentiable(self, mpm_model, mpm_state, next_state, dt, device="cuda:0"):
        ...
        # grid update
        with wp.ScopedTimer("grid_update", synchronize=True, print=False, dict=self.time_profile):
            wp.launch(
        ...
        with wp.ScopedTimer("apply_BC_on_grid", synchronize=True, print=False, dict=self.time_profile):
        ...
        # g2p
        with wp.ScopedTimer("g2p", synchronize=True, print=False, dict=self.time_profile):
            wp.launch(
    ...
    def p2g2p(self, mpm_model, mpm_state, step, dt, device="cuda:0"):
        ...
        #     input()
        #### CFL check ####
        with wp.ScopedTimer("clip_particle_x", synchronize=True, print=False, dict=self.time_profile):
            wp.launch(
            ...
            if time >= param.start_time and time < param.end_time:
                ...
                if dotproduct < 0.0:
                    if param.surface_type == 0:
                        ...
                        if float(grid_z) * model.dx < 0.4 or float(grid_z) * model.dx > 0.53:
                            state.grid_v_out[grid_x, grid_y, grid_z] = wp.vec3(0.0, 0.0, 0.0)
                        else:
                            v_in = state.grid_v_out[grid_x, grid_y, grid_z]
                            state.grid_v_out[grid_x, grid_y, grid_z] = wp.vec3(v_in[0], 0.0, v_in[2]) * 0.3
                    else:
                        ...
                        if normal_component < 0.0 and wp.length(v) > 1e-20:
                            v = wp.max(0.0, wp.length(v) + normal_component * param.friction) * wp.normalize(
                                v
                            )  # apply friction here
                        state.grid_v_out[grid_x, grid_y, grid_z] = wp.vec3(0.0, 0.0, 0.0)
    ...
    def add_impulse_on_particles(
        ...
        @wp.kernel
        def apply_force(time: float, dt: float, state: MPMStateStruct, param: Impulse_modifier):
            p = wp.tid()
            if time >= param.start_time and time < param.end_time:
                if param.mask[p] == 1:
                    impulse = wp.vec3(
    ...
    def enforce_particle_velocity_rotation(
        ...
        horizontal_1 = (
            horizontal_1 - wp.dot(horizontal_1, velocity_modifier_params.normal) * velocity_modifier_params.normal
        )
            ...
            if time >= velocity_modifier_params.start_time and time < velocity_modifier_params.end_time:
                if velocity_modifier_params.mask[p] == 1:
                    ...
                    horizontal_distance = wp.length(
                        offset - wp.dot(offset, velocity_modifier_params.normal) * velocity_modifier_params.normal
                    )
