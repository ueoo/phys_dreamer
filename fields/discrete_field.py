...
from field_components.encoding import TemporalKplanesEncoding, TriplanesEncoding
from field_components.mlp import MLP
from operators.rotation import quaternion_to_matrix, rotation_6d_to_matrix
...
class PointSetMotionSE3(nn.Module):
    ...
    def __init__(
        ...
        # init parameters:
        translation = nn.Parameter(torch.zeros(num_frames + 1, inp_x.shape[0], 3).requires_grad_(True))
        rotation = nn.Parameter(torch.ones((num_frames + 1, inp_x.shape[0], self.output_dim - 3)).requires_grad_(True))
    ...
    def compute_arap_loss(
        ...
        for i in self.inp_time_list:
            ...
            # [N, 3, 3] => [N, topk, 3, 3]
            r_mat_1to0 = r_mat_1to0.unsqueeze(1).repeat(1, self.knn_index.shape[1], 1, 1)
            dsp_t1_to_0 = torch.matmul(r_mat_1to0, dsp_t1[:, :, :, None]).squeeze(-1)
            # compute the arap loss
            arap_loss += torch.mean(torch.norm(dsp_t0 - dsp_t1_to_0, dim=-1) * self.distance_weight)
        return arap_loss
    ...
    def compute_isometry_loss(
        ...
        for i in self.inp_time_list:
            ...
            iso_loss += torch.mean(torch.abs(distance_mat - self.precompute_isometry) * self.distance_weight)
        return iso_loss
