...
class TemporalKplanesOffsetfields(nn.Module):
    ...
    def __init__(
        ...
        self.temporal_kplanes_encoding = TemporalKplanesEncoding(resolutions, feat_dim, init_a, init_b, reduce)

        self.add_spatial_triplane = add_spatial_triplane
        if add_spatial_triplane:
            self.spatial_kplanes_encoding = TriplanesEncoding(resolutions[:-1], feat_dim, init_a, init_b, reduce)
            feat_dim = feat_dim * 2
    ...
    def forward(self, inp: Float[Tensor, "*bs 4"]) -> Tuple[Float[Tensor, "*bs 3 3"], Float[Tensor, "*bs 3"]]:
    ...
    def compute_smoothess_loss(
        ...
        temporal_smoothness_loss = self.temporal_kplanes_encoding.compute_temporal_smoothness()
