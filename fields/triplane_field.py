...
from field_components.encoding import TriplanesEncoding
...
class TriplaneFields(nn.Module):
    ...
    def forward(self, inp: Float[Tensor, "*bs 3"]) -> Tuple[Float[Tensor, "*bs 3 3"], Float[Tensor, "*bs 3"]]:
        # shift to [-1, 1]
...
class TriplaneFieldsWithEntropy(nn.Module):
    ...
    def forward(self, inp: Float[Tensor, "*bs 3"]) -> Tuple[Float[Tensor, "*bs 3 3"], Float[Tensor, "1"]]:
        # shift to [-1, 1]
        inpx = SceneBox.get_normalized_positions(inp, self.aabb) * 2.0 - 1.0
