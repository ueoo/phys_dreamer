...
def compute_gradient_loss(pred, gt, mask=None):
    ...
    if mask is None:
        ...
        gradient_difference = torch.abs(pred_grad - gt_grad).mean(dim=1, keepdim=True)[mask].sum() / (
            mask.sum() + 1e-8
        )
...
def compute_low_res_psnr(img1, img2, scale_factor):
    ...
    Args:
        ...
        scale_factor: int
    """
    img1 = F.interpolate(img1, scale_factor=1 / scale_factor, mode="bilinear", align_corners=False)
    img2 = F.interpolate(img2, scale_factor=1 / scale_factor, mode="bilinear", align_corners=False)
