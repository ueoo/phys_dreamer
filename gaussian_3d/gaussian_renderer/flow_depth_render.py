...
def render_flow_depth_w_gaussian(
    ...
    point_disp_pad = torch.cat([point_disp, torch.zeros_like(point_disp[:, 0:1])], dim=-1)  # [N, 4]
    ...
    pts_motion_xy_pixel = cam_plane_2_img.unsqueeze(0) @ pts_motion_xy.unsqueeze(-1)  # [N, 2, 1]
    pts_motion_xy_pixel = pts_motion_xy_pixel.squeeze(-1)  # [N, 2]

    colors_precomp = torch.cat([pts_motion_xy_pixel, depth.unsqueeze(dim=-1)], dim=-1)  # [N, 3]
