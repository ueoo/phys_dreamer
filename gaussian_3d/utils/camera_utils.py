...
from gaussian_3d.scene.cameras import Camera
from gaussian_3d.utils.general_utils import PILtoTorch
from gaussian_3d.utils.graphics_utils import fov2focal
...
def create_cameras_around_sphere(
    ...
    cam_list = []
    dummy_image = torch.tensor(np.zeros((3, resolutions[0], resolutions[1]), dtype=np.uint8))
    for i in range(num_cams):
        R, T = extrinsics[i]
